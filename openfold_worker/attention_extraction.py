...
class AttentionCapture:
    ...
    Captures are grouped by *invocation*: one call of the layer that owns the
    `mha` module (e.g. TriangleAttention.forward). One invocation can call
    `_attention` several times, because OpenFold splits large inputs into
    chunks (chunk_utils.chunk_layer); the chunks are glued back together in
    `_save_family`.

    A layer is invoked several times per prediction:
      - Pairformer layers once per recycling iteration (4 passes), plus extra
        trial calls of block 0 while the chunk size is auto-tuned. Only the
        latest invocation is kept: the final recycle, i.e. the attention that
        produced the returned structure.
      - Diffusion transformer layers once per denoising step; every step is
        kept so `_save_family` can pick a representative one.
    """

    def __init__(self, families: set[str]):
        self.families = families
        # qualified mha name -> invocations -> chunks
        self.invocations: dict[str, list[list[np.ndarray]]] = {}
        self._context = threading.local()
        self._orig_attention = None

    def _start_invocation(self, layer_name: str) -> None:
        calls = self.invocations.setdefault(layer_name, [])
        if _family_for_name(layer_name) != "diffusion_token":
            calls.clear()  # keep memory bounded: only the latest pass matters
        calls.append([])

    def _record(self, layer_name: str, weights: np.ndarray) -> None:
        if not self.invocations.get(layer_name):
            self._start_invocation(layer_name)
        self.invocations[layer_name][-1].append(weights)
    ...
    def _make_invocation_hook(self, layer_name: str):
        def _invocation_hook(module, args, kwargs):
            self._start_invocation(layer_name)

        return _invocation_hook

    def register(self, model: torch.nn.Module) -> int:
        """Registers pre/post hooks on every Attention submodule that
        belongs to one of the requested families, and an invocation hook on
        the layer that owns it. Returns the number of hooked layers."""
        count = 0
        for name, module in model.named_modules():
            ...
            owner_name = name.rsplit(".", 1)[0]  # "...tri_att_start.mha" -> "...tri_att_start"
            model.get_submodule(owner_name).register_forward_pre_hook(
                self._make_invocation_hook(name), with_kwargs=True
            )
            count += 1
        return count
    ...
    def patch(self):
        ...
        def _spying_attention(query, key, value, biases, use_high_precision=False):
            ...
            if layer_name is not None:
                ...
                capture_self._record(layer_name, weights)
...
def triangle_heatmap(cube: np.ndarray) -> np.ndarray:
    """Reduces an [I, J, K] triangle-attention cube to an [I, K] map.

    The softmax runs over k: for every pair (i, j), w[i, j, :] sums to 1.
    Averaging over k would therefore give exactly 1/K everywhere. Averaging
    over the query index j instead gives, for each row i, how much the pairs
    (i, *) attend to pair (i, k): each row is still a distribution over k,
    comparable to a self-attention map.
    """
    return cube.astype(np.float32).mean(axis=-2).astype(np.float16)


def _assemble_chunks(chunks: list[np.ndarray], trailing_dims: int) -> np.ndarray:
    """Glues the `_attention` calls of one invocation back into a single map.

    chunk_layer flattens the batch dims (batch x I for triangle attention)
    and slices that flat axis into consecutive chunks, so reshaping each
    chunk to [-1, *trailing] and concatenating in call order restores the
    full map. Unchunked calls ([1, I, J, K] or [1, N, N]) go through the
    same path.
    """
    trailing = chunks[0].shape[-trailing_dims:]
    return np.concatenate([chunk.reshape(-1, *trailing) for chunk in chunks], axis=0)


def _save_family(
    output_dir: Path,
    family: str,
    invocations: dict[str, list[list[np.ndarray]]],
) -> dict:
    ...
    triangular_attention.py::TriangleAttention.forward). Both the full
    [I, J, K] cube and an [I, K] heatmap (see `triangle_heatmap`) are saved
    for these families so the UI can offer either view.
    """
    layer_names = [
        name for name in invocations if _family_for_name(name) == family and invocations[name]
    ]
    ...
    if family == "diffusion_token":
        # One invocation per denoising step; keep the middle step as a
        # representative snapshot (the full rollout is too much to store).
        chosen = [invocations[name][len(invocations[name]) // 2] for name in layer_names]
    else:
        # Only the latest invocation is kept (final recycle, see AttentionCapture).
        chosen = [invocations[name][-1] for name in layer_names]

    # Triangle maps are [I, J, K] (two trailing dims per row), others [N, N].
    arrays = [_assemble_chunks(chunks, 2 if is_triangle else 1) for chunks in chosen]

    shapes = {arr.shape for arr in arrays}
    if len(shapes) != 1:
        raise ValueError(f"{family}: blocks have different shapes after assembly: {sorted(shapes)}")

    if is_triangle:
        cube_stacked = np.stack(arrays, axis=0)  # [n_blocks, I, J, K]
        np.savez_compressed(output_dir / f"{family}_cube.npz", weights=cube_stacked)

        heatmaps = [triangle_heatmap(arr) for arr in arrays]
...
def extract(
    ...
    for family in requested_families:
        meta["families"][family] = _save_family(output_dir, family, capture.invocations)
